...
function Navbar() {
  ...
  return (
    <>
      <div
        style={scrolled ? { top: "-50%" } : { top: "0%", filter: blur("100px") }}
        className='fixed text-white z-[999] bg-zinc-900 w-full py-8 px-12 flex items-center justify-between duration-100 ease-In-Out'
      ></div>

      <div
        style={scrolled ? { top: "-50%" } : { top: "0%", filter: blur("100px") }}
        className='fixed text-white z-[999] w-full py-4 px-12 flex items-center justify-between duration-100 ease-linear'
      >
        ...
        <div className='links flex gap-10'>
          {
            ["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
              <a key={index} className={`font-["overpass"] cursor-pointer hover:underline ${index === 4 && "ml-40"}`}>{item}</a>
            ))
          }
        </div>
